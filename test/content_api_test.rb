...
describe GdsApi::ContentApi do
  ...
  describe "when a role is specified" do
    before do
      @base_api_url = Plek.current.find("contentapi")
      @role_api = GdsApi::ContentApi.new(
        @base_api_url,
        role: 'odi'
      )
    end
    ...
    it "returns the right url" do
      add_role = @role_api.send(:add_role, "#{@base_api_url}/batman.json")
      assert_equal "#{@base_api_url}/batman.json?role=odi", add_role
    end

    it "returns the right url when a parameter is present" do
      add_role = @role_api.send(:add_role, "#{@base_api_url}/batman.json?foo=bar")
      assert_equal "#{@base_api_url}/batman.json?foo=bar&role=odi", add_role
    end
  end
  ...
  describe "search" do
    it "should show search results" do
      ...
      assert_equal "OKFestival", response['results'][0]['title']
